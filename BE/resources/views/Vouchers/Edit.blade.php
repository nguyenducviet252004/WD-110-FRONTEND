...
@section('content_admin')
    ...
    <form method="POST" action="{{ route('vouchers.update', $voucher->id) }}" enctype="multipart/form-data" class="container">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="title" id="title" value="{{ old('title', $voucher->title) }}">
            @error('title')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" class="form-control" name="start_date_time" id="start_date_time" value="{{ old('start_date_time', $voucher->start_date_time ? \Carbon\Carbon::parse($voucher->start_date_time)->format('Y-m-d\TH:i') : '') }}">
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" class="form-control" name="end_date_time" id="end_date_time" value="{{ old('end_date_time', $voucher->end_date_time ? \Carbon\Carbon::parse($voucher->end_date_time)->format('Y-m-d\TH:i') : '') }}">
            @error('end_date_time')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" class="form-control" name="discount" id="discount" value="{{ old('discount', $voucher->discount) }}">
            @error('discount')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" class="form-control" name="min_order_amount" id="min_order_amount" value="{{ old('min_order_amount', $voucher->min_order_amount) }}">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" name="max_usage" id="max_usage" value="{{ old('max_usage', $voucher->max_usage) }}">
            @error('max_usage')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="is_active" class="form-control mt-2" id="is_active">
                <option value="1" {{ old('is_active', $voucher->is_active) == 1 ? 'selected' : '' }}>Đang hoạt động</option>
                <option value="0" {{ old('is_active', $voucher->is_active) == 0 ? 'selected' : '' }}>Không hoạt động</option>
            </select>
            @error('is_active')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="mt-3 mb-3 text-center">
            <button type="submit" class="btn btn-outline-success">Cập nhật Voucher</button>
